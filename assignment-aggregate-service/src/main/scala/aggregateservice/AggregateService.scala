...
class AggregateService extends Actor with ActorLogging {
  ...
  def doAggregate(function: String, values: String, valueType: String): AggregateResponse = {
    valueType match {
      case "long" =>
        val longAction: AggregateAction = new AggregateActionLong
        function match {
      ...
      case "double" =>
        val doubleAction: AggregateAction = new AggregateActionDouble
        function match {
      ...
      case "bigdecimal" =>
        val bigDecimalAction: AggregateAction = new AggregateActionBigDecimal
        function match {
